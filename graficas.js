...
(() => {
  ...
  class NotaGrafica {
    ...
    render() {
      ...
      const lineCount = text => (text.match(/.{1,26}(?:\s|$)|.{1,26}/g)||['']).length;
      this.rowSizes=data.map(r=>Math.max(r.values.length*42,lineCount(r.label)*16+16));
      const width=800, height=type==='barras'?Math.max(240,this.rowSizes.reduce((a,b)=>a+b,0)+112):400;
    ...
    bars(svg,width,height) {
      ...
      data.forEach((r,j)=>r.values.forEach((v,i)=>{
        ...
        if(i===0) {
          const words=r.label.match(/.{1,26}(?:\s|$)|.{1,26}/g)||[''];
        ...
        label(svg,v<0?sx(v)-8:sx(v)+8,y+16,fmt(v),{'text-anchor':v<0?'end':'start'});
      }));
    }
    lines(svg,sx,sy,b) {
