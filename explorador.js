...
(()=>{'use strict';const instances=new WeakMap(),normal=s=>s.normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase();
 function init(root=document){return [...(root.matches?.('[data-explorador]')?[root]:[]),...root.querySelectorAll('[data-explorador]')].map(el=>{
  ...
  function update(){parts.splice(0).forEach(p=>p.remove());initial.remove();
   ...
   visible.sort((a,b)=>direction*(column===3?Number(a.cells[3].dataset.valor)-Number(b.cells[3].dataset.valor):a.cells[column].textContent.localeCompare(b.cells[column].textContent,'es',{numeric:true,sensitivity:'base'}))||rows.indexOf(a)-rows.indexOf(b));
   ...
   headers.forEach((h,i)=>h.setAttribute('aria-sort',i===column?(direction===1?'ascending':'descending'):'none'));
  }
  ...
  headers.forEach((h,i)=>h.querySelector('button')?.addEventListener('click',()=>{direction=column===i?-direction:1;column=i;update();},{signal:abort.signal}));
  const instance={update,destroy(){abort.abort();parts.forEach(p=>p.remove());rows.forEach(r=>initial.append(r));table.append(initial);headers.forEach((h,i)=>originalSort[i]===null?h.removeAttribute('aria-sort'):h.setAttribute('aria-sort',originalSort[i]));status.textContent=before;instances.delete(el);}};
