...
call('goto','--url','about:blank');call('goto','--url','http://127.0.0.1:8768/examples/generated/library.html');call('exec','--command','set viewport 1440 960');evaluate('document.fonts.ready.then(()=>true)')
...
r=evaluate('''(()=>{document.querySelector('[data-ir=configuracion]').click();const e=document.querySelector('[data-ir=configuracion]'),n=document.querySelector('.navegacion-scroll'),r=e.getBoundingClientRect(),c=n.getBoundingClientRect();return {left:r.left,right:r.right,boxLeft:c.left,boxRight:c.right,scroll:n.scrollLeft,current:e.getAttribute('aria-current')}})()''');assert r['left']>=r['boxLeft']-1 and r['right']<=r['boxRight']+1 and r['current']=='page',r;save('editorial-navigation.json',r)
for key in ['cards-trazadas','atencion','estanteria','invitacion','lista-estados','pie-editorial']:
 ...
 call('screenshot');Path('tests/screenshots/editorial-'+key+'.png').write_bytes(base64.b64decode(call('screenshot')['data']))
