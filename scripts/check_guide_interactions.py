...
for item in json.loads((ROOT/'packages/core/registry/registry.json').read_text())['componentes']:assert sources.get(item['id'])==item['html'],item['id']
...
for w,h in [(320,740),(390,844),(1440,960)]:
 call('exec','--command',f'set viewport {w} {h}');evaluate('document.getElementById("guia-demo-apuntes-ejemplo").scrollIntoView({block:"center",behavior:"instant"});true');(ROOT/'tests/screenshots'/f'guia-notas-{w}.png').write_bytes(base64.b64decode(call('screenshot')['data']))
