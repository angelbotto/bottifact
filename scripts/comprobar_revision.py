...
def fresh(hash=''):call('goto','--url','about:blank');call('goto','--url',BASE+'/biblioteca.html'+hash);evaluate('document.fonts.ready.then(()=>true)')
def settle():evaluate('new Promise(r=>setTimeout(()=>r(true),150))');call('screenshot')
fresh();call('exec','--command','set viewport 390 844');r=file('pruebas_revision.js');save('revision-interaccion.json',r);assert not r['failed'],r
fresh();records=[]
...
registry=json.loads(Path('registro.json').read_text())
...
print('Revisión: ocho interacciones, 18 combinaciones, 57 fuentes intactas y escritura al entrar/reduce correctas')
