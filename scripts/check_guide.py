...
from contract_artifact import THEMES
URL='http://127.0.0.1:8768/'
call('goto','--url',URL+'examples/generated/guide.html');call('exec','--command','set viewport 1440 960');evaluate('document.fonts.ready.then(()=>true)')
pages=evaluate('[...document.querySelectorAll("main > .pagina")].map(x=>x.id)')
assert len(pages)==13,pages
assert evaluate('document.querySelectorAll("[data-guia-componente]").length')==len(json.loads((ROOT/'packages/core/registry/registry.json').read_text())['componentes'])
rows=[];errors=[]
...
for name,style in [('liftit','sobrio'),('blueprint','tecnico'),('hacker','tecnico')]:
 ...
 for w,h in [(320,740),(390,844),(1440,960)]:
  call('exec','--command',f'set viewport {w} {h}');call('screenshot');d=evaluate((ROOT/'scripts/measure_browser.js').read_text().replace("closed.forEach(d=>d.open=true)","closed.splice(0,closed.length,...closed.filter(d=>!d.closest('.pagina')||d.closest('.pagina').classList.contains('viva')));closed.forEach(d=>d.open=true)"))
  ...
  evaluate('scrollTo(0,0);true');(ROOT/'tests/screenshots'/f'guia-tema-{name}-{w}.png').write_bytes(base64.b64decode(call('screenshot')['data']))
