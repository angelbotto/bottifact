...
INDEX=json.loads((ROOT/'packages/core/themes/index.json').read_text())
DATA={k:v for k,v in INDEX.items() if k!='families'}
DATA['familias']=[json.loads((ROOT/'packages/core/themes/families'/(name+'.json')).read_text()) for name in INDEX['families']]
...
MARK='/* GENERATED THEMES — scripts/themes.py; edit packages/core/themes/families/. */'
...
def generate():
 (ROOT/'packages/core/themes/themes.json').write_text(json.dumps(DATA,ensure_ascii=False,indent=2)+'\n')
 css=(ROOT/'packages/core/styles/artifact.css').read_text().split(MARK)[0].rstrip()+'\n'
 ...
 (ROOT/'packages/core/styles/artifact.css').write_text(css+'\n'+'\n'.join(out)+'\n')
 js=(ROOT/'packages/core/components/reader.js').read_text()
 ...
 (ROOT/'packages/core/components/reader.js').write_text(js)
 md=(ROOT/'docs/components.md').read_text()
 def menu(match):
