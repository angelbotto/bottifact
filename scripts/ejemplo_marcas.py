...
from marcas import BRANDS, logo
from ejemplo_ejecutivo import document
...
def generate():
    ...
    (ROOT/'marcas.html').write_text(build('Bottifact · Identidades de marca', [{'id':'marcas','titulo':'Liftit, Tikin y Catabum','html':overview},{'id':'uso','titulo':'Uso y actualización','html':document('marcas.md','marcas-uso')}], description='Identidad trazable para documentos que se pueden compartir.', document_id='bottifact-marcas', theme='editorial', mode='system'))
    ...
    catabum += '<p class="procedencia">Identidad: tokens de la app y logo compartido. El modo claro es una adaptación para lectura. <a href="marcas.html#marca-catabum">Ver la procedencia</a>.</p>'
    for key, title, content in [('tikin','Tikin · Lectura financiera',tikin),('catabum','Catabum · Lectura de comunidad',catabum)]:
        (ROOT/'ejemplos'/('tema-'+key+'.html')).write_text(content+'\n')
        (ROOT/(key+'.html')).write_text(build(title,[{'id':'contenido','titulo':title,'html':content}],theme=key,marca=key,mode='system',style='sobrio',document_id='bottifact-'+key))
    print('Generados marcas.html, tikin.html y catabum.html con logos incrustados.')
