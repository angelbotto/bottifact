...
import re
from contract_artifact import ROOT,recipe,build
...
def generate():
 ...
 content='''<section id="revision-juntos"><p class="ceja">Laboratorio · revisión y datos</p><h2>Una conversación junto a la evidencia.</h2><p id="cita-revision">Antes de cerrar una decisión, conviene revisar la fuente, el alcance y quién se encarga del siguiente paso.</p><p>Activa la burbuja, deja un comentario sobre esta frase y abre la lista: puedes responder, asignar, resolver y exportar la revisión. Se guarda en este navegador. Para otra persona, comparte el HTML y el archivo de revisión; importar reúne los eventos.</p><p>Los nombres son declarados. Esta muestra no tiene cuentas ni sincronización remota.</p></section><section id="tabla-trabajo"><h2>Una tabla para trabajar.</h2><p>El total corresponde a las filas filtradas; la selección se mantiene al cambiar de página. Los grupos cuentan sólo las filas de su página.</p></section>'''+table+'''<section id="siguiente-capa"><h2>Lo que viene después.</h2><p>El portal del NAS ya conserva comentarios por versión, identidad y permisos. Consulta docs/portal-operations.md para conectarlo. Presencia, seguir la lectura y aprobación de versiones siguen pendientes; este HTML abierto por sí solo conserva revisión local.</p><p><a href="docs/architecture.md">Evaluación del stack</a> · <a href="docs/collaboration.md">Contrato de colaboración</a> · <a href="examples/generated/linear-light.html">Linear Light</a> · <a href="examples/generated/linear-dark.html">Linear Dark</a></p></section>'''
 (ROOT/'examples/content/collaborative-content.html').write_text(content)
 for theme,name in [('linear-light','collaborative'),('linear-light','linear-light'),('linear-dark','linear-dark')]:
  (ROOT/'examples/generated'/(name+'.html')).write_text(build('Bottifact · Revisión compartida',[{'id':'contenido','titulo':'Revisión','html':content}],theme=theme,style='sobrio',document_id='nota-laboratorio-colaborativo'))
