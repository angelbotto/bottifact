...
<a href="examples/generated/themes.html" data-audio-hover><h3>{families} familias, {version['paletas']} paletas</h3><p>La identidad visual y el modo claro, oscuro o sistema se eligen por separado.</p><span class="procedencia">HIGHLIGHT · VERSION.json</span></a>
<a href="docs/installation.md" data-audio-hover><h3>Una entrada para tres agentes</h3><p>Claude, Codex y Hermes pueden usar el mismo paquete y las mismas instrucciones.</p><span class="procedencia">HIGHLIGHT · compatibilidad de formato</span></a>
<a href="docs/collaboration.md" data-audio-hover><h3>Ingreso de invitados por verificar</h3><p>El dominio público ya está activo y la revisión conectada reside en el NAS. Falta probar la recepción real del código de acceso con otra persona.</p><span class="procedencia">LOWLIGHT · verificación externa pendiente</span></a>
<a href="docs/skill-evaluation.md" data-audio-hover><h3>Falta evaluar la voz por agente</h3><p>Una instalación válida no demuestra que los tres agentes entreguen textos equivalentes.</p><span class="procedencia">LOWLIGHT · prueba pendiente</span></a>
<a href="#metodo" data-audio-hover><h3>Impacto aún sin medir</h3><p>No hay aquí una medición de adopción, tiempo ahorrado o calidad de decisiones.</p><span class="procedencia">LOWLIGHT · vacío de evidencia</span></a>
    ...
    def chart(key, attr, title, rows, caption):
        ...
        return source
    body += chart('barras', 'capitulos', 'Recetas por capítulo', [(labels[key], value) for key,value in chapters.most_common()], 'Fuente: <a href="packages/core/registry/registry.json">packages/core/registry/registry.json</a>. '+str(count)+' recetas, una entrada por ID; corte de esta versión. Unidad: recetas. La longitud compara cantidades por capítulo.')
    ...
    body += chart('torta', 'composicion', 'Composición del catálogo', groups, 'Fuente: <a href="packages/core/registry/registry.json">packages/core/registry/registry.json</a>. Denominador: '+str(count)+' recetas. Agrupación: gráficas + reportes + tablas; lectura + expresión; los demás capítulos conservan su categoría. Cambia entre torta y donut para consultar las mismas cantidades.')
    ...
    rows = [
        ...
        ('Familias y modos', 'Biblioteca', 'Disponible', 'docs/themes.md'),
        ('Formato de tres agentes', 'Distribución', 'Disponible', 'docs/installation.md'),
        ('Calidad editorial por agente', 'Evaluación', 'Pendiente', 'docs/skill-evaluation.md'),
        ('Sincronización de comentarios', 'Colaboración', 'Pendiente', 'docs/collaboration.md'),
        ('Impacto en trabajo del equipo', 'Evaluación', 'Pendiente', 'docs/executive-voice.md'),
    ]
...
</ol><p>Responsable y fecha: por definir. Criterios de revisión: <a href="docs/skill-evaluation.md">evaluación del skill</a>.</p></section>
...
<div><dt>Inventario</dt><dd>Conteo de entradas en packages/core/registry/registry.json; familias y paletas declaradas en VERSION.json. No son métricas de productividad.</dd></div>
...
<nav class="biblioteca-rutas ancho" aria-label="Continuar"><a href="#perfil"><strong>Cómo quiero comunicar</strong><small>Voz, estructura, datos y componentes.</small></a><a href="#fuentes"><strong>Lecturas y alcance</strong><small>Fuentes primarias y adaptación del criterio.</small></a><a href="examples/generated/guide.html"><strong>Biblioteca completa</strong><small>Consultar todas las recetas.</small></a></nav>'''
    pages = [
