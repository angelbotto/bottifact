...
def generate():
 ...
 liftit+=intro('limite','Qué decidir después','Antes de atribuir una mejora al servicio, confirma el período, la fuente y si los intentos fallidos están incluidos. El tema adapta azul #0051F4 y tinta #2A2D46 medidos en liftit.co; las fuentes siguen siendo las de Bottifact.')
 ...
 for key,title,style,content in [('liftit','Liftit · Lectura de operación','sobrio',liftit),('blueprint','Blueprint · Plano de una confirmación','tecnico',blue),('hacker','Hacker · Contrato, código y evidencia','tecnico',hack)]:
  ...
  (ROOT/(key+'.html')).write_text(build(title,[{'id':'contenido','titulo':title,'html':content}],brand='nota',theme=key,style=style))
 print('Generados liftit.html, blueprint.html y hacker.html')
