"""Matriz visual con Orca: servidor scripts/servir.py --puerto 8768. No requiere paquetes."""
import json,base64
from comprobar_navegador import ROOT,call,evaluate,save
from temas import FAMILIES
from contrato_artefacto import build,recipe
fixture=ROOT/'auditoria/temas-prueba.html'
...
url='http://127.0.0.1:8768/auditoria/temas-prueba.html'
try:
 call('goto','--url',url);call('exec','--command','set media light')
 evaluate('localStorage.removeItem("nota-apariencia-v2:/auditoria/temas-prueba.html");true')
 call('goto','--url',url)
 checks=[]
...
menu.open=false;probe.remove();return rows})()'''.replace('FAMILIES',json.dumps(FAMILIES)))
 ...
 call('goto','--url','http://127.0.0.1:8768/temas.html')
