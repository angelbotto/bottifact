...
checks=evaluate('''(async()=>{const results=[],assert=(b,m)=>{if(!b)throw Error(m)},wait=ms=>new Promise(r=>setTimeout(r,ms)),test=async(name,f)=>{try{await f();results.push({name,ok:true})}catch(e){results.push({name,ok:false,error:e.message})}},go=id=>document.querySelector('.catalogo-indice a[href="#receta-'+id+'"]').click();
 await test('71 recetas, ocho piezas nuevas y navegación con nueve destinos',()=>{assert(document.querySelectorAll('.receta-copia code').length===71,'recetas');assert(document.querySelectorAll('.barra [data-ir]').length===9,'capítulos');assert(document.querySelectorAll('.barra .nav-separador').length===8,'separadores');});
