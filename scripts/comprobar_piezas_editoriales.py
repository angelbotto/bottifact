...
BASE='http://127.0.0.1:8768/prioridades.html'
...
fresh();r=file('pruebas_piezas_editoriales.js');save('piezas-editoriales-interaccion.json',r);assert not r['failed'],r;print('Interacciones:',r,flush=True)
...
for w,h in [(320,740),(390,844),(1440,960)]:
 ...
 for theme in ['light','dark','sea','oliva','arcilla','ciruela']:
  for page in ['prioridades','piezas']:
   ...
   d=file('medir_navegador.js');d['page']=page
...
for w,h,anchor in [(320,740,'galeria-ejemplo'),(390,844,'codigo-lineas-ejemplo'),(1440,960,'avisos-animados-ejemplo'),(1440,960,'actividad-editorial-ejemplo')]:
 call('exec','--command',f'set viewport {w} {h}');evaluate('document.querySelector("[data-elegir-tema][value=dark]").click();document.getElementById('+json.dumps(anchor)+').scrollIntoView({block:"center",behavior:"instant"});true');Path('auditoria/capturas/'+anchor+'-'+str(w)+'.png').write_bytes(base64.b64decode(call('screenshot')['data']))
