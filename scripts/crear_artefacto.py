...
from contrato_artefacto import build,THEMES,STYLES
...
def main():
 ...
 p.add_argument('--tema',choices=THEMES);p.add_argument('--estilo',choices=STYLES)
 ...
 try:
  ...
  result=build(title,pages,description,theme=args.tema or (config.get('tema') if args.config else None),style=args.estilo or (config.get('estilo') if args.config else None),document_id=args.documento_id or (config.get('documento_id') if args.config else None))
