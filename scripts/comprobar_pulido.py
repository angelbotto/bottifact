...
BASE='http://127.0.0.1:8768/biblioteca.html'
def fresh():
...
def capture(name):Path('auditoria/capturas/'+name+'.png').write_bytes(base64.b64decode(call('screenshot')['data']))
fresh();call('exec','--command','set viewport 390 844');settle()
r=file('pruebas_pulido.js');save('pulido-interaccion.json',r);assert not r['failed'],r;print('Ocho contratos de interacción correctos',flush=True)
