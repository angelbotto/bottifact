...
r=file('pruebas_analitica.js');save('analitica-interaccion.json',r);print(r,flush=True);assert not r['failed'],r
records=[]
...
save('analitica-ejecucion.json',{'utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'cases':len(records),'interacciones':len(file('pruebas_analitica.js')['results']),'method':'Orca: viewport, DOM, coordenadas SVG/Three, clics/teclado sintéticos; sin lector de pantalla ni dispositivo móvil físico.'})
