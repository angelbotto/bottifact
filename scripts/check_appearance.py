...
r=file('tests_appearance.js');save('apariencia-interaccion.json',r);print(r,flush=True);assert not r['failed']
records=[]
for width,height in [(320,740),(390,844),(1639,939)]:
 ...
 Path(f'tests/screenshots/apariencia-{width}.png').write_bytes(base64.b64decode(call('screenshot')['data']))
# Preferencias guardadas no afectan notas anteriores que no incluyen estos controles.
call('goto','--url',args.url.replace('examples/generated/template.html','examples/generated/chapters.html'))
...
r=file('tests_second_batch.js');save('apariencia-regresion.json',r);print('Regresión',r['passed'],r['failed'],flush=True);assert not r['failed']
