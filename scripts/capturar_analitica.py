...
call('goto','--url','about:blank');call('goto','--url','http://127.0.0.1:8768/biblioteca.html');call('exec','--command','set media reduced-motion');call('exec','--command','set viewport 1440 960');evaluate('document.fonts.ready.then(()=>true)')
for key in ['calendario','torta','areas','caja','velas','mapa-rutas','mapa-burbujas','columnas-mapa','arcos-mapa','almacen']:
 ...
 Path('auditoria/capturas/analitica-'+key+'.png').write_bytes(base64.b64decode(call('screenshot')['data']))
call('exec','--command','set viewport 1639 730');evaluate('document.documentElement.dataset.theme="dark";document.querySelector("[data-ir=graficas]").click();scrollTo({top:0,behavior:"instant"});dispatchEvent(new Event("scroll"));true');call('screenshot');Path('auditoria/capturas/analitica-cabecera-1639.png').write_bytes(base64.b64decode(call('screenshot')['data']))
call('exec','--command','set viewport 390 844');evaluate('document.querySelector("a[href=\\"#receta-torta\\"]").click();true');call('screenshot');Path('auditoria/capturas/analitica-torta-390.png').write_bytes(base64.b64decode(call('screenshot')['data']))
call('goto','--url','about:blank');call('goto','--url','http://127.0.0.1:8768/plantilla.html');call('exec','--command','set viewport 1639 939');call('screenshot')
