...
const source=fs.readFileSync(require('node:path').join(__dirname,'../revision.js'),'utf8').split('/* Comentarios locales:')[0];const context={crypto:webcrypto};vm.createContext(context);vm.runInContext(source,context);const Store=context.NotaRevisionStore;
