...
URL='http://127.0.0.1:8768/biblioteca.html'
...
call('goto','--url','http://127.0.0.1:8768/plantilla.html');checks=file('pruebas_componentes.js');save('lectura-regresion.json',checks);assert not checks['failed'],checks
call('exec','--command','set media light');call('goto','--url',URL+'#receta-calor');call('exec','--command','set viewport 1440 960');evaluate('document.fonts.ready.then(()=>{document.querySelector("[data-elegir-tema][value=light]").click();document.getElementById("receta-calor").scrollIntoView({behavior:"instant",block:"start"});return true;})');settle();Path('auditoria/capturas/lectura-1440.png').write_bytes(base64.b64decode(call('screenshot')['data']))
call('exec','--command','set viewport 390 844');evaluate('document.querySelector("[data-ir=graficas]").click();scrollTo({top:0,behavior:"instant"});true');settle();Path('auditoria/capturas/lectura-390.png').write_bytes(base64.b64decode(call('screenshot')['data']))
