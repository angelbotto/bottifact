#!/usr/bin/env python3
"""Pruebas optativas con el navegador de Orca. Iniciar scripts/servir.py antes.
No instala dependencias. Usa sólo el CLI público y guarda evidencia en auditoria/.
"""
...
def save(name,value):(ROOT/'auditoria'/name).write_text(json.dumps(value,ensure_ascii=False,indent=2)+'\n')

if __name__=='__main__':
 parser=argparse.ArgumentParser();parser.add_argument('--url',default='http://127.0.0.1:8766/plantilla.html');args=parser.parse_args()
 call('goto','--url',args.url)
 checks=file('pruebas_componentes.js');save('componentes.json',checks)
 print(f"Componentes: {checks['passed']} pasan, {checks['failed']} fallan",flush=True)
 assert not checks['failed'],checks
 audio=file('pruebas_audio.js');save('audio.json',audio)
 ...
 for width,height in [(320,740),(390,844),(1639,939)]:
  ...
  for theme in ['light','dark','sea']:
   ...
   data=file('medir_navegador.js');measurements.append(data);save('navegador.json',measurements)
 ...
 save('ejecucion.json',{'utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'url':args.url,'browser':evaluate('navigator.userAgent'),'static':'python3 scripts/validar.py se ejecuta por separado','limitations':['No acredita audición manual ni gesto físico de activación del audio.','Context loss se ensaya con eventos sintéticos; fallback sin Three sí construye una instancia sin la dependencia.','Foco comprobado por DOM/CSS; no equivale a una sesión con lector de pantalla.']})
