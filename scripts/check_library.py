...
fresh('examples/generated/library.html');call('exec','--command','set viewport 390 844');settle()
checks=evaluate('''(async()=>{
 ...
 await test('Inventario vigente y nueve capítulos',()=>{assert(document.querySelectorAll('.pagina').length===9,'capítulos');assert(document.querySelectorAll('.receta-copia code').length===Number(document.querySelector('.pie-colofon span').textContent.match(/[0-9]+/)[0]),'recetas');});
...
fresh('examples/generated/library.html#receta-calor');settle();deep=evaluate('({page:document.querySelector(".pagina.viva").id,hash:location.hash,top:document.getElementById("receta-calor").getBoundingClientRect().top})');assert deep['page']=='graficas' and deep['hash']=='#receta-calor' and 45<=deep['top']<=150,deep
...
for w,h in [(320,740),(390,844),(1440,960)]:
 ...
 for theme in ['light','dark','sea','oliva','arcilla','ciruela']:
  for page in pages:
   ...
   data=file('measure_browser.js');data['page']=page
...
fresh('examples/generated/report.html');r=file('tests_chapters.js');save('biblioteca-regresion-capitulos.json',r);assert not r['failed'],r
fresh('examples/generated/template.html');r=file('tests_second_batch.js');save('biblioteca-regresion-catalogo.json',r);assert not r['failed'],r
fresh('examples/generated/library.html#prototipos');call('exec','--command','set viewport 1440 960');settle()
...
for w,h,page,theme,name in [(390,844,'portada','oliva','biblioteca-390'),(1440,960,'publicaciones','arcilla','biblioteca-archivo'),(1440,960,'portada','light','biblioteca-inicio')]:
 call('exec','--command',f'set viewport {w} {h}');evaluate('document.querySelector("[data-elegir-tema][value='+theme+']").click();document.querySelector("[data-ir='+page+']").click();scrollTo({top:0,behavior:"instant"});true');settle();Path('tests/screenshots/'+name+'.png').write_bytes(base64.b64decode(call('screenshot')['data']))
