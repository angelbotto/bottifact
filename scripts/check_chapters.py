...
def snapshot(name):Path('tests/screenshots/'+name+'.png').write_bytes(base64.b64decode(call('screenshot')['data']))
fresh('examples/generated/report.html');call('exec','--command','set viewport 1639 939');settle()
...
r=file('tests_chapters.js');save('capitulos-interaccion.json',r);print(r,flush=True);assert not r['failed']
# Hash después de una carga nueva, no navegación sólo dentro del DOM existente.
fresh('examples/generated/report.html#prototipo');settle();assert evaluate('document.querySelector(".pagina.viva").id')=='prototipo'
records=[]
...
fresh('examples/generated/template.html');contrasts=[]
for media in ['light','dark']:
 call('exec','--command','set media '+media)
 for width,height in [(320,740),(390,844),(1639,939)]:
  call('exec','--command',f'set viewport {width} {height}')
  for theme in ['light','dark','sea','oliva','arcilla','ciruela']:
   evaluate('document.documentElement.dataset.theme='+json.dumps(theme)+';true');data=file('measure_browser.js')
...
r=file('tests_second_batch.js');save('capitulos-regresion.json',r);assert not r['failed'],r
import tests_regressions as legacy
...
fresh('examples/generated/report.html#prototipo');call('exec','--command','set media reduced-motion');call('screenshot')
