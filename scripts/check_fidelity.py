...
call('goto','--url','about:blank');call('goto','--url','http://127.0.0.1:8768/examples/generated/library.html');call('exec','--command','set viewport 1440 960');call('exec','--command','set media light');evaluate('document.fonts.ready.then(()=>true)')
...
for w,h in [(320,740),(390,844),(1440,960)]:
 ...
 for theme in ['light','dark','sea','oliva','arcilla','ciruela']:
  ...
  for name in ['apuntes','manuscrita','galeria']:
   ...
   d=file('measure_browser.js');assert d['documentWidth']==w and not d['errors'] and not d['svgTextOverflow'],d
 ...
 for name in ['apuntes','manuscrita','galeria']:
  ...
  Path(f'tests/screenshots/fidelidad-{name}-{w}.png').write_bytes(base64.b64decode(call('screenshot')['data']))
