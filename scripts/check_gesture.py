...
call('goto','--url','about:blank');call('goto','--url','http://127.0.0.1:8768/examples/generated/library.html');call('exec','--command','set media light');call('exec','--command','set viewport 1440 960');evaluate('document.fonts.ready.then(()=>{document.querySelectorAll("[data-mano],[data-subrayar]").forEach(e=>NotaMano.get(e)?.motion.dispatchEvent(new Event("change")));return true})');call('screenshot')
r=file('tests_gesture.js');save('gesto-interaccion.json',r);print(r,flush=True);assert not r['failed'],r
records=[]
