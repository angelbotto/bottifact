...
- `python3 scripts/ensamblar.py` y `python3 scripts/validar.py`: fuentes sincronizadas, CSP, IDs, anchos, inventario y contrato 4.
- `python3 scripts/probar_contrato.py`: siete grupos, incluidas las recetas componibles y rechazo de omisiones de pestañas/búsqueda/estilos.
- `python3 scripts/comprobar_explorador_apariencia.py` (ejecutado antes de renombrarlo, con el mismo contenido): 81 combinaciones de nueve paletas × tres pestañas × 320×740, 390×844 y 1440×960; interruptor dentro del panel y viewport; 18 combinaciones de tipografía; filtro con tildes, intersección vacía, restablecimiento y 50 etiquetas de prueba con scroll. Los 40 temas extra sólo viven en el navegador de prueba.
- `python3 scripts/comprobar_tipografia.py`: 156 combinaciones, 13 páginas × seis estilos × 320/390 px. Sin desbordamiento del documento; regiones amplias con foco, nombre y desplazamiento. No repite toda la matriz de colores de las gráficas, ya que no se cambiaron sus tokens.
- `python3 scripts/comprobar_audio_salida.py`: clic y lápiz originales con clic nativo, señal de AnalyserNode después del gain maestro, volumen cero y fallo simulado de AudioContext.
- `python3 scripts/comprobar_audio_preferencia.py`: silencio durante resume demorado, cancelación de la activación, pagehide simulado y reactivación tras clic real. La primera prueba de ocultación usó document.hidden, pero el host no permite redefinirlo; se cambió a pagehide, sin afirmar una prueba de visibilidad física.
- `python3 scripts/comprobar_guia_interacciones.py`: ocho interacciones de tablas, prototipo, comentarios e inicialización; 71 HTML exactos del registro; gesto nativo de lápiz y cancelación con MQL explícito.
- `python3 scripts/probar_portabilidad.py`: dos grupos, instalación/actualización en temporal, conservación de copia anterior, verificación, generación desde otro directorio y rechazo de alteraciones.
