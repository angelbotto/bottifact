...
Ejecutado en el Mac mini, navegador de Orca y servidor local con la CSP de scripts/servir.py.

- `python3 scripts/ensamblar.py`: genera biblioteca, guías, estándares y ejemplos de temas.
- `python3 scripts/validar.py`: CSP, hashes de fuentes, módulos, referencias, anchos, regiones,
  nueve paletas, contrato 3 y cobertura exacta de las 71 recetas en la guía.
- `python3 scripts/probar_contrato.py`: seis grupos, incluida composición independiente de
  todas las recetas y rechazo de omisiones/IDs/anchos inválidos.
- `python3 scripts/probar_portabilidad.py`: dos grupos. ZIP verificado, instalación temporal,
...
- `python3 scripts/comprobar_guia.py`: **351 combinaciones**, 13 páginas × nueve paletas ×
...
- `python3 scripts/comprobar_guia_interacciones.py`: ocho interacciones de tabla, visor, revisión,
...
un párrafo sobre piezas base y la aclaración del esqueleto histórico en componentes.md. Se volvió
