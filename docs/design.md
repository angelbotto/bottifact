...
The reader inherits document tokens. Its four-tool dock separates appearance, a unified comment composer, counted review and sharing. Comments use a compact writing surface with author, privacy and send; optional type, session and anchor context are disclosed. Appearance uses a 380 px panel with a searchable single-column theme list, independent mode control and separate typography/sound tabs.
