...
Lee primero [docs/executive-voice.md](executive-voice.md). Escribe desde la posición del usuario hacia su audiencia, con conclusión, evidencia y acción; no como un asistente que le entrega un reporte. [examples/generated/executive.html](../examples/generated/executive.html) muestra un memo y el perfil completo.
...
También existe un vocabulario mínimo, documentado al inicio de docs/components.md: `.marca` para
...
Consulta `packages/core/registry/use-cases.json` si necesitas estos recorridos con IDs verificables. Cambia el orden cuando
...
el montaje en cada página. En un HTML standalone, los hilos se guardan en el navegador y se intercambian por JSON. Publicado en el portal personal de Bottifact, el mismo componente guarda comentarios centralizados con identidad y permisos; el autor los reúne en Comentarios. Conserva el documento-id y consulta [docs/collaboration.md](collaboration.md) y [docs/portal-operations.md](portal-operations.md). No atribuyas sincronización al borrador local.
...
fuentes incrustadas. Procedencia: [tests/evidence/liftit-theme-source.json](../tests/evidence/liftit-theme-source.json).
...
Consulta también [examples/generated/evidence.html](../examples/generated/evidence.html), con ocho ejemplos completos. Para un artículo,
...
3. Genera con `scripts/create_artifact.py`; valida la salida con `scripts/validate_artifact.py`.
...
5. Entrega el HTML y conserva sus fuentes. Para mantenimiento de la biblioteca ejecuta
   `scripts/build.py` y `scripts/validate.py`. El HTML contiene todo salvo la inclusión permitida de Three.

Para Hermes en otro equipo, sigue [docs/installation.md](installation.md). El skill no requiere Claude,
...
[docs/themes.md](themes.md) reúne el catálogo completo: 15 familias, 30 paletas y modo Sistema. Usa `--tema linear --modo system`, por ejemplo. La tipografía es independiente. [examples/generated/themes.html](../examples/generated/themes.html) permite comparar el mismo contenido y sus componentes.
