...
Automatic updates are **opt-in**, per user, every six hours while the computer and user scheduler are available. macOS uses a LaunchAgent (also checks when loaded); Linux uses a systemd user timer with a short randomized delay. No root privileges, cron edits, shell command evaluation, or publication token are needed. `status` reports whether the scheduler definition is installed, not whether a previous download succeeded. macOS logs live in `~/.local/state/bottifact/`; Linux logs are available through `journalctl --user` for the service shown by the timer definition.
