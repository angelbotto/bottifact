...
The installer downloads from that origin and remembers it for future updates. It creates a shared skill for Claude Code, Codex and Hermes. `bottifact actualizar` continues using the selected server. A local ZIP installation is also supported; see the README.
...
```bash
bottifact conectar --servidor https://artifacts.example.com
bottifact estado
```
...
| Updates download from another host | Re-run that instance's installer, or `bottifact actualizar --servidor https://artifacts.example.com` |
