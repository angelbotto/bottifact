...
Biblioteca editorial y skill para crear artefactos HTML con datos, notas manuscritas, revisión y navegación de lectura. **82 componentes, once temas y seis combinaciones tipográficas.** Funciona con Claude, Codex y Hermes; los HTML generados no necesitan un agente para leerse.

- `guia.html`: biblioteca documentada y recetas copiables.
- `colaborativo.html`: tabla de 36 filas y revisión local con hilos.
- `linear-light.html` / `linear-dark.html`: temas inspirados en Linear.
...
```bash
python3 scripts/crear_artefacto.py --contenido ejemplos/colaborativo-contenido.html --titulo 'Mi revisión' --documento-id mi-revision --tema linear-dark --estilo sobrio --salida /tmp/mi-revision.html
...
Para mantener la biblioteca: `python3 scripts/ensamblar.py`, `python3 scripts/validar.py`, `python3 scripts/probar_contrato.py`, `node scripts/probar_revision_store.cjs` y `python3 scripts/validar_skill.py`. Las pruebas de navegador están en `scripts/comprobar_*.py`; requieren Orca y el servidor local. CI verifica contratos y paquete, no audición humana ni lector de pantalla.

## Recursos y procedencia

Las fuentes tienen sus avisos OFL en `licencias/`. Los recursos de referencia y sus hashes están documentados en `referencia-cmrg.md` y `auditoria/`. La biblioteca mantiene los recursos aprobados durante el diseño. Los temas Linear son adaptaciones propias, no oficiales. Publicar este repositorio no concede una licencia adicional sobre recursos de terceros.
