...
(()=>{
 ...
 class Fleet{
  constructor(element){
   ...
   this.motion=matchMedia('(prefers-reduced-motion: reduce)');this.longitude=-73.5;this.latitude=4.5;
  ...
  build(){
   ...
   this.buttons={};for(const [key,label]of [['region','Colombia'],['world','Globo'],['route','Acercar ruta']]){const b=node('button','',label);b.type='button';b.setAttribute('aria-pressed',String(key==='region'));this.listen(b,'click',()=>{this.world=key==='world';this.focusRoute=key==='route';this.longitude=-73.5;this.latitude=4.5;this.fit();this.sync();});this.buttons[key]=b;this.views.append(b);}
   ...
   this.surface=node('div','flota-superficie');this.canvas=node('canvas');this.canvas.setAttribute('role','img');this.canvas.setAttribute('aria-label','Colombia resaltada en un globo. Las rutas, estados y valores están en la lista y la tabla.');this.overlay=node('div','flota-etiquetas');this.surface.append(this.canvas,this.overlay);this.stage.append(this.surface);
   ...
   this.summary=node('p','flota-resumen');this.summary.setAttribute('role','status');this.note=node('p','flota-nota','Conexiones entre ciudades, no carreteras. Los camiones recorren arcos esquemáticos. Una demo completa dura 45 segundos; el avance no mide distancia, ETA ni puntualidad.');
   this.error=node('p','grafica-error');this.error.hidden=true;this.parts=[this.header,this.toolbar,this.error,this.layout,this.summary,this.note];const before=e.firstChild;this.parts.forEach(p=>e.insertBefore(p,before));
  ...
  render(){
   ...
   const row=this.model.data.find(r=>r.id===this.selected),activeCity=city=>!this.focusRoute||city.name===row.from.name||city.name===row.to.name;this.pins.forEach((pin,i)=>pin.visible=activeCity(this.model.cities[i]));this.routes.forEach((route,i)=>route.mesh.visible=!this.focusRoute||this.model.data[i].id===this.selected);
   const used=[];this.cityLabels.forEach(({city,el})=>{el.hidden=this.world||!activeCity(city);if(el.hidden)return;const p=this.point(vector(city.lat,city.lon,1.025)),w=el.offsetWidth,h=el.offsetHeight;
   ...
   this.country.hidden=!this.world;const p=this.point(vector(4.5,-73.5,1.03));this.country.style.left=(p.x+14)+'px';this.country.style.top=(p.y-18)+'px';
   this.trucks.forEach((button,i)=>{button.hidden=this.world||(this.focusRoute&&this.model.data[i].id!==this.selected);const route=this.routes[i],t=this.position(this.model.data[i])/100,point=this.point(route.curve.getPoint(t));button.style.left=point.x+'px';button.style.top=point.y+'px';const next=this.point(route.curve.getPoint(Math.min(1,t+.01))),prev=this.point(route.curve.getPoint(Math.max(0,t-.01))),angle=Math.atan2(next.y-prev.y,next.x-prev.x);button.querySelector('svg').style.transform='rotate('+angle+'rad)';});
   this.renderer.render(this.scene,this.camera);this.frames++;
  }
  select(id){if(!this.model.data.some(r=>r.id===id))throw Error('Vehículo desconocido.');this.selected=id;this.focusRoute=true;this.world=false;this.sync();this.fit();}
  sync(){
  ...
  fallback(error){this.stop();this.renderer?.dispose();this.geometries.forEach(g=>g.dispose());this.materials.forEach(m=>m.dispose());this.renderer=null;this.stage.hidden=true;this.views.hidden=true;this.error.hidden=false;this.error.textContent='Vista 3D no disponible. Puedes consultar cada vehículo y la tabla completa.';this.error.dataset.reason=error.message;this.play.disabled=true;this.slider.disabled=true;}
  destroy(){if(this.dead)return;this.dead=true;this.stop();this.abort.abort();this.observer.disconnect();this.resize?.disconnect();this.mutation.disconnect();if(this.landImage)this.landImage.onload=null;this.geometries.forEach(g=>g.dispose());this.materials.forEach(m=>m.dispose());this.renderer?.dispose();this.parts.forEach(p=>p.remove());this.element.classList.remove('nota-flota');instances.delete(this.element);}
 }
