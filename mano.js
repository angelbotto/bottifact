...
(()=>{'use strict';
 ...
 class Mano{
  ...
  finish(){this.epoch++;this.animations.forEach(a=>a.cancel());this.animations=[];this.paths.forEach(p=>{p.style.strokeDasharray='none';p.style.strokeDashoffset='0';});}
  play(){if(this.dead)return;this.finish();if(!this.visible||this.motion.matches||document.hidden)return;this.played=true;const epoch=this.epoch,total=this.lengths.reduce((a,b)=>a+b,0);let delay=0;this.paths.forEach((p,i)=>{const length=this.lengths[i],duration=this.duration*length/total;p.style.strokeDasharray=length;p.style.strokeDashoffset=length;this.animations.push(p.animate([{strokeDashoffset:length},{strokeDashoffset:0}],{duration,delay,fill:'forwards',easing:'linear'}));delay+=duration;});Promise.all(this.animations.map(a=>a.finished)).then(()=>{if(!this.dead&&epoch===this.epoch)this.finish();}).catch(()=>{});}
