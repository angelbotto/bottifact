...
conversación, navegación y footer. HTML íntegro, criterio y límites en docs/components.md.
...
la protección para desbordes reales. Evidencia: editorial-labels-diagnostic.json.

## Verificación

- `python3 scripts/build.py` y `python3 scripts/validate.py`.
- `scripts/check_editorial.py`: tooltip, Escape, foco/evento DOM, permanencia, destroy/init,
...
- `scripts/check_pencil.py`: botón de Sonidos y repetición con clic nativo de Orca;
...
- `scripts/check_library.py`: 162 combinaciones, ocho interacciones, regreso entre
