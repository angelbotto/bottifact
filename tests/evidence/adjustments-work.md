...
aporte pregunta, consecuencia o matiz. examples/generated/priorities.html incluye ejemplos derecho e izquierdo,
...
- `python3 scripts/build.py`, `python3 scripts/validate.py`,
  `python3 scripts/test_contract.py`: fuentes, CSP, IDs, seis paletas, 71 recetas y contrato.
- `python3 scripts/check_pieces_editorial.py`: 36 combinaciones de dos páginas × seis
...
- Después de ampliar el margen interno de actividad, `python3 scripts/check_adjustments_work.py`
...
Hermes: `tests/evidence/hermes-shared.json` registra descubrimiento y lectura de instrucción/registro,
