...
1. **El ejemplo recomendado no cumplía la preferencia.** `SKILL.md` recomendaba `examples/generated/report.html`
   para informes completos. Su construcción en `scripts/build.py` cargaba apariencia pero
   no `packages/core/components/review.js` ni su montaje. Por eso partir de ese ejemplo no producía comentarios.
   Se incorporó el montaje flotante y el módulo, conservando el contenido del informe.
2. **La biblioteca y la entrega tenían verificaciones diferentes.** `scripts/validate.py`
...
- Demostraciones reproducibles `examples/generated/standard.html` y `examples/generated/standard-chapters.html`. No se añadieron recetas
...
- `python3 scripts/build.py` y `python3 scripts/validate.py`: generación y contratos locales.
- `python3 scripts/test_contract.py`: composición del inventario, una/varias páginas, controles
...
- `tests/evidence/standard-browser.json`: 36 combinaciones (dos documentos × seis temas × tres
...
- `tests/evidence/audio-output.json`: clics nativos de Orca, AnalyserNode después del volumen maestro,
...
La sincronización detectó distintos finales de línea en `packages/core/styles/fonts.css` entre los dos checkouts.
