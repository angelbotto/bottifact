...
en [auditoria/referencia.json](auditoria/referencia.json). No se siguieron destinos externos
...
1. `graficas.js`: barras, líneas, temporal con delta, dispersión e histograma. Tabla
...
4. `escena.js`: dispersión XYZ y composición por etapas, con proyección ortográfica,
   alternativa textual, controles, instancia por contenedor y ciclo de vida de `NotaGlobo`.
5. `sonido.js`: canal acotado, apagado al cargar, botón de activación y señales breves
   de acción/confirmación/atención; Web Audio sin recursos externos ni disparadores de lectura.
6. `escritura.js`: secuencia de paths SVG de una frase trazada a mano; controles de repetir
   y finalizar; texto equivalente permanente. No se simula el trazo revelando cajas de glifos.
7. Catálogo ejecutable y recetas completas en `componentes.md`, incluyendo límites, cero,
